...
| MCP integration | Project-owned server built with the official Model Context Protocol SDK; real client/server judge test invokes `guardedFetch`, normalization, policy, no-spend settlement, retry, and response verification | `packages/mcp-server`, `pnpm demo:mcp:judge` | Discover and invoke four judge tools | Done | Show command in video |
